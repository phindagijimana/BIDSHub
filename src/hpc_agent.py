"""
HPC (High-Performance Computing) Agent for BIDSHub.

Provides interface to access BIDS datasets on HPC clusters via SSH/SFTP.
Supports common HPC environments (SLURM, PBS, SGE) with BIDS data storage.
"""
...
class HPCAgent:
    """Interface to HPC clusters for BIDS dataset access via SSH/SFTP."""
    
    def __init__(self, host: str, username: str, password: str = None,
                 key_file: str = None, port: int = 22):
        """
        Initialize HPC agent with SSH connection.
        
        Args:
            host: HPC hostname (e.g., 'hpc.institution.edu')
    ...
    def connect(self) -> bool:
        ...
        try:
            ...
            logger.info(f"Connected to HPC: {self.host}")
            return True
            
        except Exception as e:
            logger.error(f"Failed to connect to HPC {self.host}: {e}")
            self._connection_active = False
            return False
    ...
    def list_bids_datasets(self, base_path: str) -> List[Dict]:
        """
        List available BIDS datasets on HPC.
        
        Args:
            base_path: Base directory to search for BIDS datasets (e.g., '/data/bids/')
            
        Returns:
            List of dataset dicts with name and path
        """
        datasets = []
        ...
        try:
            ...
            stdin, stdout, stderr = self.ssh_client.exec_command(
                f'find {base_path} -name "dataset_description.json" -type f 2>/dev/null'
            )
    ...
    def get_subjects_with_metadata(self, dataset_path: str) -> List[Dict]:
        """
        Fetch subject list with metadata from HPC BIDS dataset.
        
        Args:
            dataset_path: Path to BIDS dataset on HPC
    ...
    def download_file(self, remote_path: str, local_path: str,
        ...
        """
        Download a file from HPC via SFTP.
        
        Args:
            remote_path: Full path on HPC
    ...
    def download_subject(self, dataset_path: str, subject_id: str,
        ...
        """
        Download all data for a subject from HPC.
        
        Args:
            dataset_path: BIDS dataset path on HPC
    ...
    def upload_file(self, local_path: str, remote_path: str, 
                   progress_callback: Callable[[int, int], None] = None) -> bool:
        """
        Upload a file to HPC via SFTP (v3.1.1+).
...
# Helper function
def check_hpc_availability() -> bool:
    """
